...
/**
 * triggerAiExplainer — schedules 3 silent re-fetches after a scan so that
 * AI explanations (generated async by the Lambda) appear in the UI automatically.
 * The explainer takes ~15-20s; polls at 10s, 22s, 40s to cover all cases.
 * Only patches the aiExplanation spans — no full re-render flicker.
 */
function triggerAiExplainer(module) {
  const POLL_DELAYS = [10000, 22000, 40000];
  POLL_DELAYS.forEach(delay => {
    setTimeout(async () => {
      try {
        const fresh = await fetchRisks(module);
        fresh.forEach(r => {
          if (!r.aiExplanation) return;
          // Find the rendered card for this risk and patch only the AI span
          const card = document.querySelector(`[data-risk-id="${r.resourceId}"]`);
          if (!card) return;
          const aiSpan = card.querySelector('.ai-explanation-text');
          if (aiSpan && aiSpan.dataset.filled !== 'true') {
            aiSpan.innerHTML = escHtml(r.aiExplanation);
            aiSpan.dataset.filled = 'true';
            aiSpan.style.color = '';
            aiSpan.style.fontStyle = '';
          }
        });
        console.info(`[ai-explainer] patch pass at +${delay/1000}s for module=${module}`);
      } catch (e) {
        console.warn('[ai-explainer] poll failed:', e.message);
      }
    }, delay);
  });
}
...
function renderRiskCards(risks, containerId, filterPriority = 'All') {
  ...
  container.innerHTML = filtered.map((r, i) => `
    <div class="risk-card priority-${(r.riskPriority || 'Low').toLowerCase()}" data-risk-id="${escHtml(r.resourceId || '')}" style="animation-delay:${i * 0.05}s">
      ...
      <div class="risk-details" id="risk-detail-${containerId}-${i}">
        ...
        <div class="ai-explanation-box">
          ...
          <span class="ai-explanation-text" ${r.aiExplanation ? 'data-filled="true"' : ''} style="${r.aiExplanation ? '' : 'color:var(--muted);font-style:italic'}">${r.aiExplanation ? escHtml(r.aiExplanation) : '&#9203; AI explanation generating&hellip; will appear here automatically.'}</span>
        </div>
      </div>
    </div>`).join('');
}
