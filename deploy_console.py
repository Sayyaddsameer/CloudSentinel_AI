...
def _zip_module(module_dir: Path, zip_path: Path) -> bytes:
    """Zip all .py files in the module dir + the entire shared/ folder.

    Every scanner Lambda imports from shared.scan_events and shared.schemas.risk_record.
    Without shared/ in the ZIP, Lambda raises ModuleNotFoundError and returns 502.
    """
    shared_dir = ROOT / "shared"
    with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zf:
        # Module-level .py files at the ZIP root
        for py_file in module_dir.glob("*.py"):
            zf.write(py_file, py_file.name)
        # shared/ package — preserves subdirectory structure (e.g. shared/schemas/)
        if shared_dir.exists():
            for shared_file in shared_dir.rglob("*.py"):
                arcname = shared_file.relative_to(ROOT)
                zf.write(shared_file, arcname)
 ...
 *
 * Auto-generated by deploy_console.py on {__import__('datetime').datetime.now(__import__('datetime').timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}
