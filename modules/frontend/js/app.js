    ...
    <div class="risk-card priority-${(r.riskPriority || 'Low').toLowerCase()}" style="animation-delay:${i * 0.05}s">
      ...
      <div class="risk-details" id="risk-detail-${containerId}-${i}">
        ...
        <div class="ai-explanation-box">
          ...
          <span>${r.aiExplanation ? escHtml(r.aiExplanation) : '<em style="color:var(--muted)">⏳ AI explanation is being generated by Groq… check back in a few seconds or click Rescan.</em>'}</span>
        </div>
      </div>
    </div>`).join('');
}
